...
obs
//ObservableKW(observable=io.reactivex.internal.operators.observable.ObservableFromArray@1b9367c1)
```

```kotlin
val flow = Flowable.fromArray(1, 2, 3, 4, 5).k()
flow
//FlowableKW(flowable=io.reactivex.internal.operators.flowable.FlowableFromArray@2be8284a)
```

```kotlin
val subject = PublishSubject.create<Int>().k()
subject
//ObservableKW(observable=io.reactivex.subjects.PublishSubject@3770f988)
```
...
```kotlin
obs.value()
//io.reactivex.internal.operators.observable.ObservableFromArray@1b9367c1
```

```kotlin
flow.value()
//io.reactivex.internal.operators.flowable.FlowableFromArray@2be8284a
```

```kotlin
subject.value()
//io.reactivex.subjects.PublishSubject@3770f988
```
